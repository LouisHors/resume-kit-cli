...
- 实时音视频场景经验：长期围绕 ZegoExpress SDK / ZegoLiveRoom SDK 做客户方案设计和落地支持，覆盖推拉流、连麦、混流、SEI、音频前处理、自定义采集/渲染、媒体播放器、合唱同步等能力。
...
可突出成果：
- 面向字节跳动、网易、好未来、SOUL、会玩、富聊、微博、Yalla、传音等多个 KA 客户完成跨场景 RTC 方案交付。
...
- 将音视频问题排查经验整合到智能排障系统，形成可复用的排障模板和复用材料。
...
- 能够基于 SDK 日志、设备环境和音频链路分析定位 RTC 音频体验问题，并沉淀优化方案。
- 能够基于客户接口规范实现 RTC SDK 再封装、接口适配和联调验证。
- 能够基于 AI 工具，将内部经验进行文档沉淀，工具沉淀，实现内部工作效率提升。
